...
int main(int argc, char **argv) {
    ...
    while ((opt = getopt(argc, argv, OPTIONS)) != -1) {
        ...
        case 'e': command = insert_set(HEAP, command); break;

        case 'i': command = insert_set(INSERTION, command); break;

        case 's': command = insert_set(SHELL, command); break;

        case 'q': command = insert_set(QUICK, command); break;
